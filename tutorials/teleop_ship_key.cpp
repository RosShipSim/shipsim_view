...
private:
  ...
  void goalResponseCallback(std::shared_future<rclcpp_action::ClientGoalHandle<shipsim::action::RotateAbsolute>::SharedPtr> future);
  void cancelGoal();

  rclcpp::Node::SharedPtr nh_;
  double linear_, angular_, l_scale_, a_scale_;
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr twist_pub_;
  rclcpp_action::Client<shipsim::action::RotateAbsolute>::SharedPtr rotate_absolute_client_;
  rclcpp_action::ClientGoalHandle<shipsim::action::RotateAbsolute>::SharedPtr goal_handle_;
};

TeleopShip::TeleopShip() : linear_(0),
                           angular_(0),
                           l_scale_(2.0),
                           a_scale_(2.0)
{
  nh_ = rclcpp::Node::make_shared("teleop_ship");
  ...
  twist_pub_ = nh_->create_publisher<geometry_msgs::msg::Twist>("ship1/cmd_vel", 1);
  rotate_absolute_client_ = rclcpp_action::create_client<shipsim::action::RotateAbsolute>(nh_, "ship1/rotate_absolute");
}

void TeleopShip::sendGoal(float theta)
{
  auto goal = shipsim::action::RotateAbsolute::Goal();
  goal.theta = theta;
  auto send_goal_options = rclcpp_action::Client<shipsim::action::RotateAbsolute>::SendGoalOptions();
  send_goal_options.goal_response_callback =
      [this](std::shared_future<rclcpp_action::ClientGoalHandle<shipsim::action::RotateAbsolute>::SharedPtr> future) {
...
void TeleopShip::goalResponseCallback(std::shared_future<rclcpp_action::ClientGoalHandle<shipsim::action::RotateAbsolute>::SharedPtr> future)
{
  RCLCPP_DEBUG(nh_->get_logger(), "Goal response received");
  this->goal_handle_ = future.get();
}

void TeleopShip::cancelGoal()
{
  if (goal_handle_)
  {
...
int TeleopShip::keyLoop()
{
  char c;
  bool dirty = false;

  std::thread{std::bind(&TeleopShip::spin, this)}.detach();

  puts("Reading from keyboard");
  puts("---------------------------");
  puts("Use arrow keys to move the ship.");
